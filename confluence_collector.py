...
def scrape_to_folder(scraping_config: ScrapingConfig, out_path: Path = Path('out'), log_verbose: bool = False):
    ...
    file_count = write_output_files(collection.threat_modelings, out_path, log_verbose)
...
def write_output_files(threat_modelings: List[ScrapedThreatModeling], out_path: Path, log_verbose: bool = False):
    if log_verbose:
        print(f"Output: Writing files to path `{out_path}` ..")
    file_count = 0
    for app, values in group_per_app(threat_modelings).items():
        write_yaml_file(out_path, app, values, log_verbose)
        file_count += 1
    return file_count
