...
from model import DateTitleLinks, \
    Link, Activities, DSOMMapplication, Settings, DateTitleLinksComponent
...
def parse_dete_title_links(page):
...
def to_date_title_link(page, space_mapping):
    meta = parse_dete_title_links(page)
    subject = space_mapping.get(meta['space'], Subject.unmapped_space(meta['space']))
    return ScrapedDateTitleLinks(subject,
                                 meta['title'],
                                 meta['date'],
...
def to_component(m: ScrapedDateTitleLinks):
    return DateTitleLinksComponent(date=m.date, title=m.title, links=m.links)
...
def write_yaml_file(folder, subject, content, log_verbose=False):
    output_filename = f"{folder}/{subject.team_name}/{subject.application_name}_application.yaml"
    if log_verbose:
        print(f"File I/O: Preparing folder/file `{output_filename}` for {len(content)} content(s) ..")
    os.makedirs(f"{folder}/{subject.team_name}", exist_ok=True)

    components = [DateTitleLinksComponent(date=m.date, title=m.title, links=m.links) for m in content]
    c = DateTitleLinks(components=components)
    a = Activities(
        date_title_link=c)
    ...
    filedata = filedata.replace("date_title_link", cfg["activityName"])
    with open(output_filename, 'w') as file:
        file.write(filedata)

@dataclass
class CollectionResult:
    date_title_links: List[ScrapedDateTitleLinks]
    errors: List[str]


def collect_date_title_links(pages, application_map, log_verbose=False):
    collected = []
    errors = []
    for p in pages:
        try:
            tm = to_date_title_link(p, application_map)
            if log_verbose:
                print(f"* {repr(tm)}")
            collected.append(tm)
...
def per_app(dateTitleLinks):
    # group per app
    grouped = {}
    for t in dateTitleLinks:
...
if __name__ == "__main__":
    ...
    page_label_to_search = cfg["label"]
    is_verbose = True
    out_path = 'out'
    ...
    if is_verbose:
        print(f"Scraping: Collecting {page_label_to_search} ..")
    collection = collect_date_title_links(found_pages, space_to_application_map, log_verbose=is_verbose)
    print(f"Scraping: Collected {len(collection.date_title_links)} {page_label_to_search}.")
    ...
    file_count = 0
    for app, values in per_app(collection.date_title_links).items():
