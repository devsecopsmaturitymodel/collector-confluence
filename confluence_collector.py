...
class ScrapingConfig:
    ...
    activity_name: str = "Threat Modeling"
...
def scrape_to_folder(scraping_config: ScrapingConfig, out_path: Path = Path('out'), log_verbose: bool = False):
    ...
    file_count = write_output_files(collection.threat_modelings, out_path, scraping_config.activity_name, log_verbose)
...
def write_output_files(threat_modelings: List[ScrapedThreatModeling], out_path: Path, activity_name, log_verbose: bool = False):
    if log_verbose:
        print(f"Output: Writing files to path `{out_path}` ..")
    file_count = 0
    for app, values in group_per_app(threat_modelings).items():
        write_yaml_file(out_path, app, values, activity_name, log_verbose)
...
def write_yaml_file(folder: Path, subject: Subject, threat_modelings: List[ThreatModeling], activity_name,
                    log_verbose: bool = False):
